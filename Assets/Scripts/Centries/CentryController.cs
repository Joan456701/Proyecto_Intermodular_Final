...
public class CentryController : InventoryHolder, IDamagable, IInteractable, ITargetable
{
    ...
    private void Shoot()
    {
        Vector3 direction = (actualObjective.transform.position - _raycastPivot.position).normalized;

        if (Physics.Raycast(_raycastPivot.position, direction, out RaycastHit hit))
        {
